...
@Component({
  ...
  imports: [CommonModule, RouterModule, FormsModule, FontAwesomeModule, SharedModule, SortDirective, SortByDirective],
})
export class NamedQueryComponent implements OnInit {
  ...
  delete(namedQuery: INamedQuery): void {
    const modalRef = this.modalService.open(NamedQueryDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.namedQueryService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
